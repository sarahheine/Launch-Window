function[phase] = mphase(jd, ras,ram,decs,decm, distancemoonkm)

%Below is the header for the IDL file mphase.pro 
%Modifications by Phil Oakley (1/10/2012).
% INPUT:
%       JD - Julian date, scalar or vector, double precision recommended
%       ras - Right Ascension of the Sun (radians)
%       ram - Right Ascension of the Moon (radians)
%       decs - Declination of the Sun (radians)
%       decm - Declination of the Moon (radians)
%       distancemoonkm - distance to the moon in km
...
% ;-
 ...
 diss = 1.49598e8;       %Earth-Sun distance (1 AU) %%in km
...
% inc = atan( diss * sin(phi) / (distancemoonkm - diss*cos(phi)) );
 inc = atan2( diss * sin(phi) , (distancemoonkm - diss*cos(phi)) );
 phase = (1 + cos(inc))/2.;
